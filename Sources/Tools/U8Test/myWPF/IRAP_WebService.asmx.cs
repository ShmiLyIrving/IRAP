...
namespace myWPF
{
    ...
    public class IRAP_WebService : System.Web.Services.WebService
    {
        [WebMethod]
        public string IRAPUES(string ExCode)
        {
            //ExCode = "<Parameters><Param ExCode=\"Q001\" MPHoneNo=\"13913911904\" CityCode=\"3208\" /></Parameters>";
            XmlDocument doc = new XmlDocument();
            doc.LoadXml(ZipUtil.UnZip(ExCode));
            //doc.LoadXml(ExCode);
            XmlElement ss = (System.Xml.XmlElement)doc.SelectNodes("Parameters/Param")[0];

            string Mode = ss.GetAttribute("Mode").ToString();
            System.String sVouchType = ss.GetAttribute("sVouchType").ToString();
            System.Object domPosition = ss.GetAttribute("domPosition").ToString();

            System.String VouchId = ss.GetAttribute("VouchId").ToString();
            System.Boolean bCheck = bool.Parse(ss.GetAttribute("bCheck").ToString());
            System.Boolean bBeforCheckStock = bool.Parse(ss.GetAttribute("bBeforCheckStock").ToString());
            System.Boolean bIsRedVouch = bool.Parse(ss.GetAttribute("bIsRedVouch").ToString());
            System.String sAddedState = ss.GetAttribute("sAddedState").ToString();
            System.Boolean bReMote = bool.Parse(ss.GetAttribute("bReMote").ToString());
            System.String sAccID = ss.GetAttribute("sAccID").ToString();
            string body = ss.GetAttribute("body").ToString();
            XmlDocument doc2 = new XmlDocument();
            doc2.LoadXml(body);
            body = doc2.OuterXml;
            string head = ss.GetAttribute("head").ToString();
            doc2.LoadXml(head);
            head = doc2.OuterXml;
            bool s = false;
            ...
            if (Mode == "MaterialOutAdd")
            {
                s = MaterialOutAdd(sAccID       //账套编号 
                    ,sVouchType,                //单据类型 
                    head,                       //表头xml
                    body,                       //表体xml
                    domPosition,                //货位，默认为空
                    ...
                    out o,
                    bCheck,                     //是否控制可用量
                    bBeforCheckStock,           //是否检查可用量
                    bIsRedVouch,                //是否红字单据
                    sAddedState,                //空
                    bReMote);                   //是否远程传入，false
            }
            else if (Mode == "ProductInAdd")
            {
                s = ProductInAdd(sAccID
                    ,sVouchType,
                    head,
                    body,
                    domPosition,
            ...
            else if (Mode == "PuStoreInAdd")
            {
                s = PuStoreInAdd(sAccID
                    , sVouchType,
                    head,
                    body,
                    domPosition,
            ...
            else if (Mode == "TransVouchAdd")
            {
                s = TransVouchAdd(sAccID,
                    sVouchType,
                    head,
                    body,
                    domPosition,
            ...
            else if (Mode == "otheroutAdd")
            {
                s = otheroutAdd(sAccID,
                    sVouchType,
                    head,
                    body,
                    domPosition,
            ...
            string domMsgstr = "";
            if (domMsg != null)
                domMsgstr = (o as MSXML2.IXMLDOMDocument2).xml.ToString();
            if (s)
            {
                result = $"<Result><Param  ErrCode=\"0\" ErrText=\"{errMsg}\" domMsgO=\"{domMsgstr}\"/></Result>";
            }
            else
            {
                result = $"<Result><Param  ErrCode=\"9999\" ErrText=\"{errMsg}\" domMsgO=\"{domMsgstr}\"/></Result>";
            }
            return ZipUtil.Zip(result);
        }


        //材料出库单

        public bool MaterialOutAdd(System.String saccID,
        ...
        public bool ProductInAdd(System.String saccID,
         System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
        ...
        public bool PuStoreInAdd(System.String saccID,
        System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
        ...
        public bool TransVouchAdd(System.String saccID,
         System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
        ...
        System.Boolean bReMote)
        {
            try
            {
                ...
                WriteLog.Instance.Write("登录成功","登录");
                //第二步：构造环境上下文对象，传入login，并按需设置其它上下文参数
                U8EnvContext envContext = new U8EnvContext();
                envContext.U8Login = u8Login;
                ...
                {
                    WriteLog.Instance.Write(ex.Message, "为表头赋值");
                }
                //DomHead.RowCount = 1; //设置BO对象(表头)行数，只能为一行
                ////给BO对象(表头)的字段赋值，值可以是真实类型，也可以是无类型字符串
                ////以下代码示例只设置第一行值。各字段定义详见API服务接口定义
                ...
                {
                    WriteLog.Instance.Write(ex.Message, "为表体赋值");
                }
                //domBody.RowCount = 10; //设置BO对象行数
                ////可以自由设置BO对象行数为大于零的整数，也可以不设置而自动增加行数
                ////给BO对象的字段赋值，值可以是真实类型，也可以是无类型字符串
                ...
                {
                    WriteLog.Instance.Write(ex.Message, "调用API");
                }
                //第七步：获取返回结果
        ...
        public bool saleoutAdd(System.String saccID,
         System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
        ...
        public bool otherinAdd(System.String saccID,
         System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
        ...
        public bool otheroutAdd(System.String saccID,
        System.String sVouchType,
        string DomHeadXml,
        string domBodyXml,
        System.Object domPosition,
