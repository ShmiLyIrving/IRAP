...
namespace IRAP.BL.Kanban
{
    public class IRAPKanban : IRAPBLLBase
    {
            ...
            out string errText)
        {
            ...
            try
            {
                List<SelectedLeafEx> datas = new List<SelectedLeafEx>();
                ...
                try
                {
                    ...
                    {
                        string strSQL = "SELECT * " +
                            "FROM IRAP..sfn_SelectedLeafSetEx(" +
                            "@CommunityID, @TreeID, @ClickStream, @LanguageID)" +
                            "ORDER BY Ordinal";

                        IList<SelectedLeafEx> lstDatas = 
                            conn.CallTableFunc<SelectedLeafEx>(strSQL, paramList);
                ...
                {
                    errCode = 99000;
                    errText = string.Format("调用 IRAP..sfn_SelectedLeafSetEx 函数发生异常：{0}", error.Message);
                    WriteLog.Instance.Write(errText, strProcedureName);
                    WriteLog.Instance.Write(error.StackTrace, strProcedureName);
                }
                #endregion
