...
namespace IRAP.BL.MES
{
    public class SmeltingProduction : IRAPBLLBase
    {
        ...
        /// <param name="t107LeafID">工位叶标识</param>
        /// <param name="batchNumber">熔次号</param>
        /// <param name="rSFactXML">工艺参数XML
        ///     &lt;RSFact&gt;
        ///         &lt;RF13_1&gt;
        ///             &lt;Ordinal="" T101LeafID="" LotNumber=""  Qty=""/&gt;
        ///         &lt;/RF13_1&gt;
        ///     &lt;/RSFact&gt;
        /// </param>
        /// <param name="sysLogID">系统登录标识</param>
        ...
        /// <param name="sysLogID">语言标识</param>
        /// <returns></returns>
        public IRAPJsonResult ufn_GetList_SmeltBatchMaterial(
            int communityID, 
            int t101LeafID, 
            long sysLogID, 
            out int errCode, 
            out string errText)
        {
            ...
            try
            {
                List<SmeltBatchMaterial> data = 
                    new List<SmeltBatchMaterial>();
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用函数 ufn_GetList_SmeltBatchMaterial,参数：" +
                        "CommunityID={0}|T101LeafID={1}||SysLogID={2}",
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                using (IRAPSQLConnection conn = new IRAPSQLConnection())
                {
                    IRAPError error = conn.CallProc("IRAPMES..usp_SaveFact_SmeltBatchInspecting", ref paramList);
                    errCode = error.ErrCode;
                    errText = error.ErrText;
                    WriteLog.Instance.Write(string.Format("({0}){1}", errCode, errText), strProcedureName);
            ...
            {
                errCode = 99000;
                errText = string.Format("调用 IRAPMES..usp_SaveFact_SmeltBatchInspecting 时发生异常：{0}", error.Message);
                return Json("");
            }
            finally
            {
