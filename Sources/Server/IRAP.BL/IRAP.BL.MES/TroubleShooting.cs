...
namespace IRAP.BL.MES
{
    public class TroubleShooting : IRAPBLLBase
    {
        ...
        /// <returns>List[FailureSrcOperation]</returns>
        public IRAPJsonResult ufn_GetKanban_FaileureSrcOperation(
            int communityID,
            long sysLogID,
            int productLeaf,
            int workUnitLeaf,
            ...
            out string errText)
        {
            ...
            try
            {
                List<FailureSrcOperation> datas = new List<FailureSrcOperation>();
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用函数 IRAPMES..ufn_GetKanban_FaileureSrcOperation，" +
                        "参数：CommunityID={0}|ProductLeaf={1}|WorkUnitLeaf={2}|" +
                        "SysLogID={3}",
                        communityID, productLeaf, workUnitLeaf, sysLogID),
                    strProcedureName);
                ...
                try
                {
                    ...
                    {
                        string strSQL = "SELECT * " +
                            "FROM IRAPMES..ufn_GetKanban_FaileureSrcOperation(" +
                            "@CommunityID, @SysLogID, @ProductLeaf, " +
                            "@WorkUnitLeaf) " +
                            "ORDER BY Ordinal";

                        IList<FailureSrcOperation> lstDatas =
                            conn.CallTableFunc<FailureSrcOperation>(strSQL, paramList);
                        datas = lstDatas.ToList();
                        errCode = 0;
                        errText = string.Format("调用成功！共获得 {0} 条记录", datas.Count);
                        WriteLog.Instance.Write(errText, strProcedureName);
                ...
                catch (Exception error)
                {
                    ...
                    errText =
                        string.Format(
                            "调用 IRAPMES..ufn_GetKanban_FaileureSrcOperation 函数发生异常：{0}",
                            error.Message);
                    WriteLog.Instance.Write(errText, strProcedureName);
                    WriteLog.Instance.Write(error.StackTrace, strProcedureName);
                }
        ...
        /// <param name="productLeaf">产品叶标识</param>
        /// <param name="workUnitLeaf">工位叶标识</param>
        /// <returns>List[FailureNature]</returns>
        public IRAPJsonResult ufn_GetKanban_FailureNature(
            int communityID,
            long sysLogID,
            int productLeaf,
            int workUnitLeaf,
            ...
            out string errText)
        {
            ...
            try
            {
                List<FailureNature> datas = new List<FailureNature>();
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用函数 IRAPMES..ufn_GetKanban_FailureNature，" +
                        "参数：CommunityID={0}|ProductLeaf={1}|WorkUnitLeaf={2}|" +
                        "SysLogID={3}",
                        communityID, productLeaf, workUnitLeaf, sysLogID),
                    strProcedureName);
                ...
                try
                {
                    ...
                    {
                        string strSQL = "SELECT * " +
                            "FROM IRAPMES..ufn_GetKanban_FailureNature(" +
                            "@CommunityID, @SysLogID, @ProductLeaf, " +
                            "@WorkUnitLeaf) " +
                            "ORDER BY Ordinal";

                        IList<FailureNature> lstDatas =
                            conn.CallTableFunc<FailureNature>(strSQL, paramList);
                        datas = lstDatas.ToList();
                        errCode = 0;
                        errText = string.Format("调用成功！共获得 {0} 条记录", datas.Count);
                        WriteLog.Instance.Write(errText, strProcedureName);
                ...
                catch (Exception error)
                {
                    ...
                    errText =
                        string.Format(
                            "调用 IRAPMES..ufn_GetKanban_FailureNature 函数发生异常：{0}",
                            error.Message);
                    WriteLog.Instance.Write(errText, strProcedureName);
                    WriteLog.Instance.Write(error.StackTrace, strProcedureName);
                }
        ...
        /// <param name="productLeaf">产品叶标识</param>
        /// <param name="workUnitLeaf">工位叶标识</param>
        /// <returns>List[FailureDuty]</returns>
        public IRAPJsonResult ufn_GetKanban_FailureDuties(
            int communityID,
            long sysLogID,
            int productLeaf,
            int workUnitLeaf,
            ...
            out string errText)
        {
            ...
            try
            {
                List<FailureDuty> datas = new List<FailureDuty>();
                ...
                WriteLog.Instance.Write(
                    string.Format(
                        "调用函数 IRAPMES..ufn_GetKanban_FailureDuties，" +
                        "参数：CommunityID={0}|ProductLeaf={1}|WorkUnitLeaf={2}|" +
                        "SysLogID={3}",
                        communityID, productLeaf, workUnitLeaf, sysLogID),
                    strProcedureName);
                ...
                try
                {
                    ...
                    {
                        string strSQL = "SELECT * " +
                            "FROM IRAPMES..ufn_GetKanban_FailureDuties(" +
                            "@CommunityID, @SysLogID, @ProductLeaf, " +
                            "@WorkUnitLeaf) " +
                            "ORDER BY Ordinal";

                        IList<FailureDuty> lstDatas =
                            conn.CallTableFunc<FailureDuty>(strSQL, paramList);
                        datas = lstDatas.ToList();
                        errCode = 0;
                        errText = string.Format("调用成功！共获得 {0} 条记录", datas.Count);
                        WriteLog.Instance.Write(errText, strProcedureName);
                ...
                catch (Exception error)
                {
                    ...
                    errText =
                        string.Format(
                            "调用 IRAPMES..ufn_GetKanban_FailureDuties 函数发生异常：{0}",
                            error.Message);
                    WriteLog.Instance.Write(errText, strProcedureName);
                    WriteLog.Instance.Write(error.StackTrace, strProcedureName);
                }
            ...
            out string errText)
        {
            ...
            try
            {
                ...
                try
                {
                    transactNo = UTS.Instance.msp_GetTransactNo(
                        communityID,
                        1,
                        "IRAPMES..stb010",
                        userCode,
                ...
                {
                    long factID = UTS.Instance.msp_GetSequenceNo("NextFactNo", 1);
                    ...
                    {
                        //新申请事实编号
                        factID = UTS.Instance.msp_GetSequenceNo("NextFactNo", 1);
                        //保存报废主事实
