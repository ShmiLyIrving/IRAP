...
namespace IRAP.Global
{
    ...
    public class FTPClient
    {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="server">FTP 服务器地址或名称</param>
        /// <param name="user">登录账号</param>
        /// <param name="pass">登陆密码</param>
        ...
        public FTPClient(string server, int port, string user, string pass, int mode, int timeout_sec)
        {
            ...
            bucket = "";
            bytes_total = 0;
            this.timeout = (timeout_sec <= 0) ? int.MaxValue : (timeout_sec * 1000);    //超时时间
            messages = "";
            errormessage = "";
        }
        #endregion
